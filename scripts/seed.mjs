...
try {
  ...
  for (const item of products) {
    const result = await database.query(`INSERT INTO products (category_id, title, slug, description, price_cents, installments, active) VALUES ((SELECT id FROM categories WHERE slug=$1), $2, $3, $4, $5, $6, TRUE) ON CONFLICT (slug) DO UPDATE SET category_id=EXCLUDED.category_id, title=EXCLUDED.title, description=EXCLUDED.description, price_cents=EXCLUDED.price_cents, installments=EXCLUDED.installments, updated_at=NOW() RETURNING id`, [item.category, item.title, item.slug, item.description, item.price, item.installments]);
